...
class FakeSandbox:
    ...
    Doubles as both provider and live handle (`create()` returns self) so
    these tests can share one instance — the orchestrator's lifecycle code
    is exercised by the dedicated lifecycle tests, not here.
    """
